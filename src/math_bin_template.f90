...
module syntran__math_BINFN_m
	...
	interface BINFN
		module procedure BINFN_value_t
	end interface BINFN
...
subroutine BINFN_value_t(left, right, res, op_text)
	...
	case        (magic**2 * array_type + magic * array_type + f32_type)
		...
		case (f32_type)
			...
			res%array%f32 = left%array%f32 BINOP right%sca%f32
		case (f64_type)
			res%array = mold(left%array, f64_type)
			res%array%f64 = left%array%f64 BINOP right%sca%f32
		case (i32_type)
			res%array = mold(left%array, f32_type)
			res%array%f32 = left%array%i32 BINOP right%sca%f32
		case (i64_type)
			res%array = mold(left%array, f32_type)
			res%array%f32 = real(left%array%i64) BINOP right%sca%f32
		case default
	...
	case        (magic**2 * array_type + magic * array_type + f64_type)
		...
		case (f32_type)
			res%array = mold(left%array, f64_type)
			res%array%f64 = left%array%f32 BINOP right%sca%f64
		case (f64_type)
			res%array = mold(left%array, f64_type)
			res%array%f64 = left%array%f64 BINOP right%sca%f64
		case (i32_type)
			res%array = mold(left%array, f64_type)
			res%array%f64 = left%array%i32 BINOP right%sca%f64
		case (i64_type)
			res%array = mold(left%array, f64_type)
			res%array%f64 = real(left%array%i64, 8) BINOP right%sca%f64
		case default
			write(*,*) err_eval_binary_types(op_text)
			call internal_error()
		end select
	...
	case        (magic**2 * array_type + magic * array_type + i32_type)
		...
		case (i32_type)
			...
			res%array%i32 = left%array%i32 BINOP right%sca%i32
		case (i64_type)
			res%array = mold(left%array, i64_type)
			res%array%i64 = left%array%i64 BINOP right%sca%i32
		case (f32_type)
			res%array = mold(left%array, f32_type)
			res%array%f32 = left%array%f32 BINOP right%sca%i32
		case (f64_type)
			res%array = mold(left%array, f64_type)
			res%array%f64 = left%array%f64 BINOP right%sca%i32
		case default
			write(*,*) err_eval_binary_types(op_text)
			call internal_error()
		end select
	...
	case        (magic**2 * array_type + magic * array_type + i64_type)
		...
		case (i32_type)
			res%array = mold(left%array, i64_type)
			res%array%i64 = left%array%i32 BINOP right%sca%i64
		case (i64_type)
			res%array = mold(left%array, i64_type)
			res%array%i64 = left%array%i64 BINOP right%sca%i64
		case (f32_type)
			res%array = mold(left%array, f32_type)
			res%array%f32 = left%array%f32 BINOP real(right%sca%i64)
		case (f64_type)
			res%array = mold(left%array, f64_type)
			res%array%f64 = left%array%f64 BINOP real(right%sca%i64, 8)
		case default
			write(*,*) err_eval_binary_types(op_text)
			call internal_error()
		end select
	...
	case        (magic**2 * array_type + magic * i32_type + array_type)
		...
		case (i32_type)
			...
			res%array%i32 = left%sca%i32 BINOP right%array%i32
		case (i64_type)
			res%array = mold(right%array, i64_type)
			res%array%i64 = left%sca%i32 BINOP right%array%i64
		case (f32_type)
			res%array = mold(right%array, f32_type)
			res%array%f32 = left%sca%i32 BINOP right%array%f32
		case (f64_type)
			res%array = mold(right%array, f64_type)
			res%array%f64 = left%sca%i32 BINOP right%array%f64
		case default
			write(*,*) err_eval_binary_types(op_text)
			call internal_error()
		end select
	...
	case        (magic**2 * array_type + magic * i64_type + array_type)
		...
		case (i32_type)
			res%array = mold(right%array, i64_type)
			res%array%i64 = left%sca%i64 BINOP right%array%i32
		case (i64_type)
			res%array = mold(right%array, i64_type)
			res%array%i64 = left%sca%i64 BINOP right%array%i64
		case (f32_type)
			res%array = mold(right%array, f32_type)
			res%array%f32 = real(left%sca%i64) BINOP right%array%f32
		case (f64_type)
			res%array = mold(right%array, f64_type)
			res%array%f64 = real(left%sca%i64, 8) BINOP right%array%f64
		case default
			write(*,*) err_eval_binary_types(op_text)
			call internal_error()
		end select
	...
	case        (magic**2 * array_type + magic * f32_type + array_type)
		...
		case (f32_type)
			res%array = mold(right%array, f32_type)
			res%array%f32 = left%sca%f32 BINOP right%array%f32
		case (f64_type)
			res%array = mold(right%array, f64_type)
			res%array%f64 = left%sca%f32 BINOP right%array%f64
		case (i32_type)
			res%array = mold(right%array, f32_type)
			res%array%f32 = left%sca%f32 BINOP right%array%i32
		case (i64_type)
			res%array = mold(right%array, f32_type)
			res%array%f32 = left%sca%f32 BINOP real(right%array%i64)
		case default
			write(*,*) err_eval_binary_types(op_text)
			call internal_error()
		end select
	...
	case        (magic**2 * array_type + magic * f64_type + array_type)
		...
		case (f32_type)
			res%array = mold(right%array, f64_type)
			res%array%f64 = left%sca%f64 BINOP right%array%f32
		case (f64_type)
			res%array = mold(right%array, f64_type)
			res%array%f64 = left%sca%f64 BINOP right%array%f64
		case (i32_type)
			res%array = mold(right%array, f64_type)
			res%array%f64 = left%sca%f64 BINOP right%array%i32
		case (i64_type)
			res%array = mold(right%array, f64_type)
			res%array%f64 = left%sca%f64 BINOP real(right%array%i64, 8)
		case default
			write(*,*) err_eval_binary_types(op_text)
			call internal_error()
		end select
	...
	case        (magic**2 * array_type + magic * array_type + array_type)
		...
		case (magic * i32_type + i32_type)
			res%array = mold(right%array, i32_type)
			res%array%i32 = left%array%i32 BINOP right%array%i32

		case (magic * i64_type + i64_type)
			res%array = mold(right%array, i64_type)
			res%array%i64 = left%array%i64 BINOP right%array%i64

		case (magic * f32_type + f32_type)
			res%array = mold(right%array, f32_type)
			res%array%f32 = left%array%f32 BINOP right%array%f32

		case (magic * i32_type + f32_type)
			res%array = mold(right%array, f32_type)
			res%array%f32 = left%array%i32 BINOP right%array%f32

		case (magic * f32_type + i32_type)
			res%array = mold(right%array, f32_type)
			res%array%f32 = left%array%f32 BINOP right%array%i32

		case (magic * i64_type + f32_type)
			res%array = mold(right%array, f32_type)
			res%array%f32 = real(left%array%i64) BINOP right%array%f32

		case (magic * f32_type + i64_type)
			res%array = mold(right%array, f32_type)
			res%array%f32 = left%array%f32 BINOP real(right%array%i64)
		...
		case (magic * f64_type + f32_type)
			res%array = mold(right%array, f64_type)
			res%array%f64 = left%array%f64 BINOP right%array%f32

		case (magic * i32_type + f64_type)
			res%array = mold(right%array, f64_type)
			res%array%f64 = left%array%i32 BINOP right%array%f64

		case (magic * f64_type + i32_type)
			res%array = mold(right%array, f64_type)
			res%array%f64 = left%array%f64 BINOP right%array%i32

		case (magic * i64_type + f64_type)
			res%array = mold(right%array, f64_type)
			res%array%f64 = real(left%array%i64, 8) BINOP right%array%f64

		case (magic * f64_type + i64_type)
			res%array = mold(right%array, f64_type)
			res%array%f64 = left%array%f64 BINOP real(right%array%i64, 8)

		case (magic * i32_type + i64_type)
			res%array = mold(right%array, i64_type)
			res%array%i64 = left%array%i32 BINOP right%array%i64
	...
	case        (magic**2 * i32_type + magic * i32_type + i32_type)
		res%sca%i32 = left%sca%i32 BINOP right%sca%i32
	...
	case        (magic**2 * i32_type + magic * i32_type + f32_type)
		res%sca%i32 = int(left%sca%i32 BINOP right%sca%f32)

	case        (magic**2 * i32_type + magic * f64_type + i32_type)
		res%sca%i32 = int(left%sca%f64 BINOP right%sca%i32)

	case        (magic**2 * i32_type + magic * i32_type + f64_type)
		res%sca%i32 = int(left%sca%i32 BINOP right%sca%f64)

	!****
	case        (magic**2 * f32_type + magic * f32_type + f32_type)
		res%sca%f32 = left%sca%f32 BINOP right%sca%f32

	case        (magic**2 * f32_type + magic * f32_type + i32_type)
		res%sca%f32 = left%sca%f32 BINOP right%sca%i32

	case        (magic**2 * f32_type + magic * f32_type + i64_type)
		res%sca%f32 = left%sca%f32 BINOP real(right%sca%i64)

	case        (magic**2 * f32_type + magic * i64_type + f32_type)
		res%sca%f32 = real(left%sca%i64) BINOP right%sca%f32

	case        (magic**2 * f32_type + magic * i32_type + f32_type)
		res%sca%f32 = left%sca%i32 BINOP right%sca%f32

	!****
	case        (magic**2 * f64_type + magic * f64_type + f64_type)
		res%sca%f64 = left%sca%f64 BINOP right%sca%f64

	case        (magic**2 * f64_type + magic * f64_type + i32_type)
		res%sca%f64 = left%sca%f64 BINOP right%sca%i32

	case        (magic**2 * f64_type + magic * f64_type + i64_type)
		res%sca%f64 = left%sca%f64 BINOP real(right%sca%i64, 8)

	case        (magic**2 * f64_type + magic * i64_type + f64_type)
		res%sca%f64 = real(left%sca%i64, 8) BINOP right%sca%f64

	case        (magic**2 * f64_type + magic * i32_type + f64_type)
		res%sca%f64 = left%sca%i32 BINOP right%sca%f64

	!****
	! Mixed f32/f64 cases
	case        (magic**2 * f64_type + magic * f64_type + f32_type)
		res%sca%f64 = left%sca%f64 BINOP right%sca%f32
...
end subroutine BINFN_value_t

!===============================================================================

end module syntran__math_BINFN_m
