...
module syntran__math_right_shift_m
	...
	interface right_shift
		module procedure right_shift_value_t
...
subroutine right_shift_value_t(left, right, res, op_text)
	...
	case        (magic**2 * array_type + magic * array_type + i32_type)
		...
		case (i32_type)
			...
			res%array%i32 = shiftr(left%array%i32, right%sca%i32)
		case (i64_type)
			res%array = mold(left%array, i64_type)
			res%array%i64 = shiftr(left%array%i64, right%sca%i32)
		case default
	...
	case        (magic**2 * array_type + magic * array_type + i64_type)
		...
		case (i32_type)
			res%array = mold(left%array, i64_type)
			res%array%i64 = shiftr(left%array%i32, right%sca%i64)
		case (i64_type)
			res%array = mold(left%array, i64_type)
			res%array%i64 = shiftr(left%array%i64, right%sca%i64)
		case default
			write(*,*) err_eval_binary_types(op_text)
			call internal_error()
		end select
	...
	case        (magic**2 * array_type + magic * i32_type + array_type)
		...
		case (i32_type)
			...
			res%array%i32 = shiftr(left%sca%i32, right%array%i32)
		case (i64_type)
			res%array = mold(right%array, i64_type)
			res%array%i64 = shiftr(left%sca%i32, right%array%i64)
		case default
			write(*,*) err_eval_binary_types(op_text)
			call internal_error()
		end select
	...
	case        (magic**2 * array_type + magic * i64_type + array_type)
		...
		case (i32_type)
			res%array = mold(right%array, i64_type)
			res%array%i64 = shiftr(left%sca%i64, right%array%i32)
		case (i64_type)
			res%array = mold(right%array, i64_type)
			res%array%i64 = shiftr(left%sca%i64, right%array%i64)
		case default
			write(*,*) err_eval_binary_types(op_text)
			call internal_error()
		end select
	...
	case        (magic**2 * array_type + magic * array_type + array_type)
		...
		case (magic * i32_type + i32_type)
			res%array = mold(right%array, i32_type)
			res%array%i32 = shiftr(left%array%i32, right%array%i32)

		case (magic * i64_type + i64_type)
			res%array = mold(right%array, i64_type)
			res%array%i64 = shiftr(left%array%i64, right%array%i64)

		case (magic * i32_type + i64_type)
			res%array = mold(right%array, i64_type)
			res%array%i64 = shiftr(left%array%i32, right%array%i64)

		case (magic * i64_type + i32_type)
			res%array = mold(right%array, i64_type)
			res%array%i64 = shiftr(left%array%i64, right%array%i32)
	...
	case        (magic**2 * i32_type + magic * i32_type + i32_type)
		res%sca%i32 = shiftr(left%sca%i32, right%sca%i32)

	case        (magic**2 * i64_type + magic * i64_type + i64_type)
		res%sca%i64 = shiftr(left%sca%i64, right%sca%i64)

	case        (magic**2 * i64_type + magic * i64_type + i32_type)
		res%sca%i64 = shiftr(left%sca%i64, right%sca%i32)

	case        (magic**2 * i64_type + magic * i32_type + i64_type)
		res%sca%i64 = shiftr(left%sca%i32, right%sca%i64)

	case        (magic**2 * i32_type + magic * i32_type + i64_type)
		res%sca%i32 = shiftr(left%sca%i32, right%sca%i64)
...
end subroutine right_shift_value_t

!===============================================================================

end module syntran__math_right_shift_m
