...
subroutine mul_value_t(left, right, res, op_text)
	...
	! `*` is element-wise multiplication.  Matrix multiplication uses the
	! separate `@` operator (see math_bin_matmul.f90).
	...
	case        (magic**2 * array_type + magic * array_type + array_type)
		...
		case (magic * f32_type + f64_type)
			!print *, "f32 * f64"
			res%array = mold(right%array, f64_type)
			res%array%f64 = left%array%f32 * right%array%f64

		case (magic * f64_type + f32_type)
			res%array = mold(right%array, f64_type)
			res%array%f64 = left%array%f64 * right%array%f32
...
end module syntran__math_mul_m
